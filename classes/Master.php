...
Class Master extends DBConnection {
	...
	public function delete_upload(){
		extract($_POST);
		 $del = $this->conn->query("DELETE FROM `post` where id = $id");
		 if($del){
			$this->settings->set_flashdata("success"," Data successfully deleted");
		 	return 1;
		 }else{
	...
	public function save_upload(){
		...
		$data = "";
		$ins_title = $this->conn->query("INSERT INTO `post` set title = '{$title}' ");
		if(!$ins_title){
			return 2;
			exit;
		}

		$ins_description = $this->conn->query("INSERT INTO `post` set mota = '{$mota}' ");
		if(!$ins_description){
			return 2;
			exit;
		}

		$ins_file = $this->conn->query("INSERT INTO `post` set file = '{$file}' ");
		if(!$ins_file){
			return 2;
			exit;
		}

		if(isset($ins_title) && isset($ins_description) && isset($ins_file)){
			$this->settings->set_flashdata("success"," Data successfully saved");
			return 1;
		}else{
			return 2;
